...
######################
def train(
    start_epoch=0, additional_epoch=100, lr=0.0001, optim="adam", leaky_relu=True,
    ndcg_gain_in_train="identity", sigma=1.0,
    double_precision=False, standardize=False,
    small_dataset=False, debug=False,
    output_dir="D:\Data_Mining\pytorch-examples-master\\ranking\\tmp\\ranking_output",
):
    ...
    #valid_loader, df_valid, train_loader, df_train = load_train_vali_data('Fold1')
    ...
    for i in range(start_epoch, start_epoch + additional_epoch):
        ...
        if debug:
            eval_cross_entropy_loss(net, device, train_loader, i, writer=writer, phase="Train")
        ...
        if i % 5 == 0 and i != start_epoch:
            ...
            eval_cross_entropy_loss(net, device, valid_loader, i, writer=writer)
            eval_ndcg_at_k(net, device, df_valid, valid_loader, 100000, [10, 30], i, writer=writer)
